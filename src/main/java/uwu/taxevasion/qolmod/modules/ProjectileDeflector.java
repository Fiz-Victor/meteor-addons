...
public class ProjectileDeflector extends Module {
    ...
    public ProjectileDeflector() {
        super(NotRat.Category, "projectile-deflector", "omg it works!");
    }
    ...
    private void onTick(TickEvent.Post event) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            if (entity instanceof ShulkerBulletEntity && Utils.inRange(entity.getPos(), range.get())) {
                mc.interactionManager.attackEntity(mc.player, entity);
            } else if (entity instanceof FireballEntity && Utils.inRange(entity.getPos(), range.get()) && isApproaching(entity)) {
                switch (mode.get()) {
                    case Up -> {
